...
void PointCloudCreateOp::cook(Foundry::Katana::GeolibCookInterface& interface)
{
	...
	{
		FnAttribute::IntAttribute generateTypeAttr = aGroupAttr.getChildByName("generateType");
		int generateType = generateTypeAttr.getValue(0, false);
		
		if (generateType == 0)
		{
			createGridPoints(interface);
		}
		else
		{
			createPointsFromFile(interface);
	...
	// if we've got a subgroup for split locations of a grid of points, handle that
...
void PointCloudCreateOp::createGridPoints(Foundry::Katana::GeolibCookInterface& interface)
{
	FnAttribute::GroupAttribute aGroupAttr = interface.getOpArg("a");
	
	FnAttribute::IntAttribute numPointsAttr = aGroupAttr.getChildByName("numPoints");

	int64_t numPoints = numPointsAttr.getValue(0, false);
	if (numPoints == 0)
	{
		return;
	}
...
//			vectorPrimvarGb.set("outputType", FnAttribute::StringAttribute("vector3"));
	...
	else
	{
		...
		// TODO: split into groups the size of two dimensions, such that
		//       we can easily calculate slice chunks of the grid positions
		//       in isolation.
...
void PointCloudCreateOp::createPointsFromFile(Foundry::Katana::GeolibCookInterface& interface)
{
	FnAttribute::GroupAttribute aGroupAttr = interface.getOpArg("a");
	
	FnAttribute::IntAttribute fileTypeAttr = aGroupAttr.getChildByName("fileType");
	int fileType = fileTypeAttr.getValue(0, false);
	
	FnAttribute::StringAttribute filePathAttr = aGroupAttr.getChildByName("filePath");
	std::string filePath = filePathAttr.getValue("", false);
	
	if (filePath.empty())
		return;
	
	// TODO: need to work out what we're doing with regard to point widths / radii / radius and if there is a
	//       difference - maybe add a scale param to control this?
	
	std::vector<Vec3> aPointPositions;
	std::vector<float> aPointWidths;
	
	bool loadedPoints = false;
	bool haveWidths = false;
	
	if (fileType == 0)
	{
		// ASCII positions
		loadedPoints = loadPositionsFromASCIIFile(filePath, aPointPositions);
	}
	else if (fileType == 1)
	{
		// ASCII positions + radii
		loadedPoints = loadPositionsAndRadiiFromASCIIFile(filePath, aPointPositions, aPointWidths);
		haveWidths = true;
	}
	...
	int64_t numPoints = aPointPositions.size();
	
	// TODO: allow optionally overriding point widths even if we read it from file...
	...
	const float* pData = &aPointPositions.data()->x;
...
bool PointCloudCreateOp::loadPositionsFromASCIIFile(const std::string& filePath, std::vector<Vec3>& aItemPositions)
{
	...
	while (fileStream.getline(buf, 256))
	{
		...
		aItemPositions.push_back(Vec3(x, y, z));
	}
...
bool PointCloudCreateOp::loadPositionsAndRadiiFromASCIIFile(const std::string& filePath, std::vector<Vec3>& aItemPositions, std::vector<float>& aItemWidths)
{
	std::ios::sync_with_stdio(false);
	std::fstream fileStream;
	std::ios::openmode mode = std::ios::in;
	...
	while (fileStream.getline(buf, 256))
	{
		...
		sscanf(buf, "%f %f %f %f", &x, &y, &z, &width);
		
		aItemPositions.push_back(Vec3(x, y, z));
		
		aItemWidths.push_back(width);
	}
